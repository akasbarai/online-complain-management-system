...
const progressForStatus = (status: ComplaintStatus) => {
  if (status === ComplaintStatus.RESOLVED || status === ComplaintStatus.CLOSED) return 'w-full bg-civic-500';
  if (status === ComplaintStatus.WITHDRAWN) return 'w-full bg-slate-400';
  if (status === ComplaintStatus.REJECTED) return 'w-full bg-red-500';
  if (
    status === ComplaintStatus.ASSIGNED ||
    status === ComplaintStatus.IN_PROGRESS ||
    status === ComplaintStatus.AWAITING_MATERIALS ||
    status === ComplaintStatus.ESCALATED
  ) return 'w-2/3 bg-primary-600';
  return 'w-1/3 bg-primary-600';
};
...
export const ComplaintDetails = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="premium-dark flex flex-col items-start justify-between gap-4 bg-[linear-gradient(135deg,#071624_0%,#102f56_100%)] p-6 md:flex-row md:items-center">
         <div>
            <h1 className="text-2xl font-bold tracking-tight text-white">{complaint.title}</h1>
            <div className="mt-2 flex items-center gap-3 text-sm text-slate-300">
               <span className="rounded bg-white/10 px-2 py-0.5 font-mono text-slate-100 ring-1 ring-white/10">#{complaint.id}</span>
               <span>•</span>
               <span>{new Date(complaint.createdAt).toLocaleDateString()}</span>
            </div>
         </div>
         <div className="flex gap-3">
            {![ComplaintStatus.CLOSED, ComplaintStatus.RESOLVED, ComplaintStatus.REJECTED, ComplaintStatus.WITHDRAWN].includes(complaint.status) && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         <div className="lg:col-span-2 space-y-6">
            <Card className="p-6">
               <div className="mb-6 flex items-center justify-between">
                  <h3 className="font-bold text-ink-950">Current Status</h3>
                  <Badge variant={
                     complaint.status === ComplaintStatus.RESOLVED ? 'success' : 
                     complaint.status === ComplaintStatus.WITHDRAWN ? 'secondary' :
               ...
               <div className="relative pt-6 pb-2">
                  <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                     <div className={`h-full rounded-full transition-all ${progressForStatus(complaint.status)}`}></div>
                  </div>
            ...
            <Card className="p-6 space-y-6">
               <div>
                  ...
                  <p className="rounded-lg border border-slate-200 bg-slate-50 p-4 leading-relaxed text-ink-900">
         ...
         <div>
            <Card className="h-full p-6">
               <h3 className="mb-6 flex items-center font-bold text-ink-950">
                 <Clock size={18} className="mr-2 text-primary-700" /> Timeline
               </h3>
               ...
               <div className="space-y-8 relative before:absolute before:left-3.5 before:top-2 before:bottom-0 before:w-0.5 before:bg-slate-200">
                  {complaint.history.map((event, index) => (
                    <div key={index} className="relative pl-10">
                       <div className="absolute left-0 top-1 z-10 flex h-7 w-7 items-center justify-center rounded-full border-2 border-primary-600 bg-white">
                          <div className="h-2.5 w-2.5 rounded-full bg-primary-600"></div>
                       </div>
                       <div>
                          <p className="text-sm font-bold text-ink-950">{event.action}</p>
