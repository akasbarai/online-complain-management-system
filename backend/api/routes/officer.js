...
const { getEscalationTargetLevel, refreshSlaBreaches, validateStatusTransition } = require('../utils/workflow');
...
router.put('/complaints/:id/escalate', async (req, res) => {
  try {
    ...
    const [complaints] = await pool.query(
      `SELECT status, user_id, department_id, current_hierarchy_level_id
       FROM complaints
       WHERE id = ? AND assigned_officer_id = ? AND is_trashed = FALSE`,
      [req.params.id, req.user.id]
    );
    if (complaints.length === 0) return res.status(404).json({ error: 'Complaint not found' });
    const complaint = complaints[0];

    const transitionError = validateStatusTransition(complaint.status, 'Escalated');
    if (transitionError) return res.status(400).json({ error: transitionError });

    const targetLevelId = await getEscalationTargetLevel(
      pool,
      complaint.department_id,
      complaint.current_hierarchy_level_id
    );
    ...
    const [result] = await pool.query(
      ...
      ['Escalated', targetLevelId, req.params.id, req.user.id]
    );
    ...
    await createNotification(pool, {
      title: `Complaint #${req.params.id} Needs Reassignment`,
      message: `${req.user.name} escalated this complaint. Reason: ${reason}`,
      target: 'Admins',
      priority: 'Urgent'
    });
