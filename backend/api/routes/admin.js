...
const COMPLAINT_STATUSES = ['Submitted', 'Under Review', 'Assigned', 'In Progress', 'Awaiting Materials', 'Escalated', 'Resolved', 'Closed', 'Rejected', 'Withdrawn'];
...
router.get('/attention', async (req, res) => {
  try {
    ...
    const [[complaintsNeedingAction]] = await pool.query(
      `SELECT COUNT(*) as count
       FROM complaints
       WHERE is_trashed = FALSE
       AND status NOT IN ('Resolved', 'Closed', 'Rejected', 'Withdrawn')
       AND (
         ...
         OR (sla_deadline IS NOT NULL AND sla_deadline < NOW())
       )`
    );
    ...
    res.json({
      ...
      complaints: complaintsNeedingAction.count,
...
const formatQueueComplaint = (c) => ({
  ...
  hierarchyName: c.hierarchy_name,
...
router.get('/workflow-queue', async (req, res) => {
  try {
    ...
    const complaintSelect = `
      SELECT c.*, u.name as user_name, d.name as department_name, o.name as officer_name, hl.name as hierarchy_name
      FROM complaints c
    ...
    const [unassignedComplaints] = await pool.query(`
      ...
      WHERE c.is_trashed = FALSE
        ...
        AND c.status NOT IN ('Resolved', 'Closed', 'Rejected', 'Withdrawn', 'Escalated')
    ...
    const [slaRiskComplaints] = await pool.query(`
      ${complaintSelect}
      WHERE c.is_trashed = FALSE
        AND c.status NOT IN ('Resolved', 'Closed', 'Rejected', 'Withdrawn')
        AND c.sla_deadline IS NOT NULL
        AND (c.sla_breached = TRUE OR c.sla_deadline <= DATE_ADD(NOW(), INTERVAL 24 HOUR))
      ORDER BY c.sla_deadline ASC
      LIMIT 25
    `);

    const [pendingUsers] = await pool.query(`
      SELECT id, name, email, mobile, address, registered_date, status
      FROM users
      WHERE status = 'Pending'
      ORDER BY registered_date ASC, created_at ASC
      LIMIT 25
    `);

    const [officerSetupIssues] = await pool.query(`
      SELECT o.id, o.name, o.email, o.department_id, d.name as department_name,
             o.hierarchy_level_id, o.jurisdiction, o.status
      FROM officers o
      LEFT JOIN departments d ON o.department_id = d.id
      WHERE o.role != 'Admin'
        AND (o.status != 'Active' OR o.hierarchy_level_id IS NULL OR o.jurisdiction IS NULL OR o.jurisdiction = '')
    ...
    res.json({
      summary: {
        ...
        slaRisk: slaRiskComplaints.length,
        pendingUsers: pendingUsers.length,
        officerSetupIssues: officerSetupIssues.length
      },
      unassignedComplaints: unassignedComplaints.map(formatQueueComplaint),
      escalatedComplaints: escalatedComplaints.map(formatQueueComplaint),
      slaRiskComplaints: slaRiskComplaints.map(formatQueueComplaint),
      ...
      officerSetupIssues: officerSetupIssues.map(o => ({
        id: o.id,
        name: o.name,
        email: o.email,
    ...
    console.error('Workflow queue error:', err);
...
router.get('/officers', async (req, res) => {
  try {
    const [officers] = await pool.query(`
      SELECT o.*,
        SUM(CASE WHEN c.id IS NOT NULL AND c.status NOT IN ('Resolved', 'Closed', 'Rejected', 'Withdrawn') AND c.is_trashed = FALSE THEN 1 ELSE 0 END) AS active_complaint_count,
        SUM(CASE WHEN c.id IS NOT NULL AND c.status = 'Assigned' AND c.is_trashed = FALSE THEN 1 ELSE 0 END) AS new_assignment_count,
        SUM(CASE WHEN c.id IS NOT NULL AND c.sla_deadline IS NOT NULL AND c.sla_deadline <= DATE_ADD(NOW(), INTERVAL 24 HOUR) AND c.status NOT IN ('Resolved', 'Closed', 'Rejected', 'Withdrawn') AND c.is_trashed = FALSE THEN 1 ELSE 0 END) AS sla_risk_count
      FROM officers o
...
router.put('/complaints/:id/status', async (req, res) => {
  try {
    ...
    const [complaints] = await pool.query(
      'SELECT status, user_id, assigned_officer_id FROM complaints WHERE id = ? AND is_trashed = FALSE',
      [req.params.id]
    );
    if (complaints.length === 0) return res.status(404).json({ error: 'Complaint not found' });
    const complaint = complaints[0];

    const transitionError = validateStatusTransition(complaint.status, status);
    ...
    await createNotification(pool, {
      title: `Complaint #${req.params.id} Status Updated`,
      message: `Your complaint status is now ${status}. ${notes || ''}`.trim(),
      target: 'Users',
      recipientType: 'User',
      recipientId: complaint.user_id,
      priority: ['Rejected', 'Closed', 'Resolved', 'Withdrawn'].includes(status) ? 'Important' : 'Normal'
    });

    if (complaint.assigned_officer_id) {
      await createNotification(pool, {
        title: `Complaint #${req.params.id} Status Updated`,
        message: `Admin changed the complaint status to ${status}. ${notes || ''}`.trim(),
        target: 'Officers',
        recipientType: 'Officer',
